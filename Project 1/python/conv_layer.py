...
def conv_layer_forward(input_data, layer, param):
    ...
    The layer for a convolutional layer has the same fields as that of a pooling layer 
    and param has the weights corresponding to the layer. 
    A convolution layer has a field “num”, which is the number of kernels, 
    which is equal to the number of output channels.
    ...
    #Use im2col_conv_batch() to convert to column form
    col = im2col_conv_batch(input_n, layer, h_out, w_out) 
    col = np.reshape(col, (k*k*c, h_out*w_out*batch_size))

    #Use the Convolution layer formula:
    #f(x,w,b) = w*x + b
    #Transposed where needed to get correct dimensions aligned with each other
    dotp = np.dot(np.transpose(col), w)
    res_t = np.transpose(dotp + b)
    res_reshaped = np.reshape(res_t, (h_out*w_out*num, batch_size)) 
    output['data'] = res_reshaped
